'''
Integrantes del Grupo: Ruiz Cinquegrani Agustin y Sarmiento Daniel.
Una juguetería tiene mucho éxito en dos de sus productos: payasos y muñecas.
Suele hacer venta por correo y la empresa de logística les cobra por peso de cada
paquete así que deben calcular el peso de los payasos y muñecas que saldrán en
cada paquete a demanda. Cada payaso pesa 112 g y cada muñeca 75 g. Escribir un
programa que lea el número de payasos y muñecas vendidos en el último pedido y
calcule el peso total del paquete que será enviado.
'''

payasos = 0
muñecas = 0
pesopayaso = 0
pesomuñecas = 0


while True:
    paquete = int(input('Ingresa 1 para payasos, ingresa 2 para muñecas, e ingresa 3 para cerrar el paquete: '))
    if paquete == 1:
        payasos += 1
        pesopayaso = pesopayaso + 112
    elif paquete == 2:
        muñecas += 1
        pesomuñecas = pesomuñecas + 75
    else:
        break

print(f'La cantidad de Payasos vendidos es: {payasos} y de Muñecas es: {muñecas} el peso total del paquete es ', (pesopayaso + pesomuñecas))

